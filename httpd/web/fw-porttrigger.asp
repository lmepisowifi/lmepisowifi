...
<%SendWebHeadStr(); %>
<title><% multilang("25" "LANG_PORT_TRIGGERING"); %><% multilang("262" "LANG_CONFIGURATION"); %></title>
...
function addClick(obj)
{
	...
	if (document.formPortTriggerAdd.localFromPort.value=="" &&  document.formPortTriggerAdd.localToPort.value=="" && document.formPortTriggerAdd.comment.value=="" ){
	    alert('<% multilang("2279" "LANG_LOCAL_SETTINGS_CANNOT_BE_EMPTY"); %>');
			return false;
	}
	...
	if (document.formPortTriggerAdd.localFromPort.value!="") {
		if ( validateKey( document.formPortTriggerAdd.localFromPort.value ) == 0 ) {
			alert('<% multilang("2282" "LANG_INVALID_PORT_NUMBER_IT_SHOULD_BE_THE_DECIMAL_NUMBER_0_9"); %>');
			document.formPortTriggerAdd.localFromPort.focus();
			return false;
		}
		d2 = getDigit(document.formPortTriggerAdd.localFromPort.value, 1);
		if (d2 > 65535 || d2 < 1) {
			alert('<% multilang("2283" "LANG_INVALID_PORT_NUMBER_YOU_SHOULD_SET_A_VALUE_BETWEEN_1_65535"); %>');
			document.formPortTriggerAdd.localFromPort.focus();
			return false;
		}
	}
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("25" "LANG_PORT_TRIGGERING"); %> <% multilang("262" "LANG_CONFIGURATION"); %></p>
	<p class="intro_content"><% multilang("3275" "LANG_ENTRIES_IN_THIS_TABLE_ARE_USED_TO_OPEN_A_PORT_AUTOMATICALLY_FOR_INCOMING_TRAFFIC_AFTER_A_PORT_HAS_BEEN_ACCESSED_BY_OUTGOING_TRAFFIC"); %></p>
</div>
...
<div class="data_common data_common_notitle">
	<table>
		<tr>
			...
			<td width="30%">
				<input type="radio" value="0" name="portTriggercap" <% checkWrite("portTrigger-cap0"); %> onClick="updateState()"><% multilang("271" "LANG_DISABLE"); %>&nbsp;&nbsp;
				<input type="radio" value="1" name="portTriggercap" <% checkWrite("portTrigger-cap1"); %> onClick="updateState()"><% multilang("272" "LANG_ENABLE"); %>
			</td>
		<td width="40%"><input class="inner_btn" type="submit" value="<% multilang("169" "LANG_APPLY_CHANGES"); %>" name="apply" onClick="return on_submit(this)">&nbsp;&nbsp;</td>
		</tr>
	</table>
</div>
<div class="data_common data_common_notitle">
<table style ="table-layout:fixed; overflow:hidden;">
    <tr>
		<th width="25%">
			<% multilang("427" "LANG_COMMENT"); %>
		</th>
		<th width="10%">
			<% multilang("3276" "LANG_TRIGGER_START_PORT"); %>
		</th>
		<th width="10%">
			<% multilang("3277" "LANG_TRIGGER_END_PORT"); %>
		</th>
		<th width="15%">
			<% multilang("101" "LANG_PROTOCOL"); %>
		</th>
 		<th width="10%">
			<% multilang("3278" "LANG_INCOMING_START_PORT"); %>
		</th>
 		<th width="10%">
			<% multilang("3279" "LANG_INCOMING_END_PORT"); %>
		</th>
 		<th>
			<% multilang("75" "LANG_INTERFACE"); %>
		</th>
    </tr>
	...
	<tr>
		...
		<td>
			<select name="protocol">
				<option select value=4><% multilang("426" "LANG_BOTH"); %></option>
...
<div class="btn_ctl">
	<input class="link_bg" type="submit" value="<% multilang("245" "LANG_ADD"); %>" name="addPortTrigger" onClick="return addClick(this)">
...
<div class="column">
	<div class="column_title">
		...
		<p> <% multilang("3280" "LANG_CURRENT_PORT_TRIGGERING_TABLE"); %></p>
...
<div class="btn_ctl">
	<input class="link_bg" type="submit" value="<% multilang("248" "LANG_DELETE_SELECTED"); %>" name="deleteSelPortTrigger" onClick="return deleteClick(this)">
	<input class="link_bg" type="submit" value="<% multilang("249" "LANG_DELETE_ALL"); %>" name="deleteAllPortTrigger" onClick="return deleteAllClick(this)">
</div>
	<script>
