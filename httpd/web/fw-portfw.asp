...
function addClick(obj)
{
	...
	for(var i=0;i<mutli_localFromPort.length;i++)
	{
		...
		if (mutli_localIPAddr[i].value!="")
		{
			...
			{				
				alert('<% multilang("2280" "LANG_INVALID_LOCAL_IP_ADDRESS"); %>');
		...
		if ( mutli_remoteIPAddr[i].value!="" ) {
            ...
            {				
				alert('<% multilang("2281" "LANG_INVALID_REMOTE_IP_ADDRESS"); %>');
...
<div class="intro_main ">
	...
	<p class="intro_content"> <% multilang("425" "LANG_PAGE_DESC_NAT_FIREWALL_FILTER_TABLE"); %></p>
</div>
...
<div class="data_common data_common_notitle">
	<table>
		<tr>
			...
			<td width="40%">
				<input type="radio" value="0" name="portFwcap" <% checkWrite("portFw-cap0"); %> onClick="updateState()"><% multilang("271" "LANG_DISABLE"); %>&nbsp;&nbsp;
				<input type="radio" value="1" name="portFwcap" <% checkWrite("portFw-cap1"); %> onClick="updateState()"><% multilang("272" "LANG_ENABLE"); %>&nbsp;&nbsp;
			</td>
			<td>
...
<div class="data_common data_common_notitle">
	<table>
		<tr>
			<th width="30%"><% multilang("272" "LANG_ENABLE"); %>
				<input type="checkbox" name="fw_enable" value="1" checked>
			</th>
			<th width="70%"><% multilang("433" "LANG_APPLICATION"); %>
...
<table style ="table-layout:fixed; overflow:hidden;">
    <tr>
		...
		<th width="15%">
			<% multilang("434" "LANG_LOCAL_IP"); %>
		</th>
		<th width="10%">
			<% multilang("435" "LANG_LOCAL_PORT_FROM"); %>
		</th>
		<th width="10%">
			<% multilang("436" "LANG_LOCAL_PORT_TO"); %>
		</th>
		<th width="10%">
			<% multilang("101" "LANG_PROTOCOL"); %>
		</th>
 		<th width="10%">
			<% multilang("437" "LANG_REMOTE_IP"); %>
		</th>
 		<th width="10%">
			<% multilang("439" "LANG_REMOTE_PORT_FROM"); %>
		</th>
 		<th width="10%">
			<% multilang("440" "LANG_REMOTE_PORT_TO"); %>
		</th>
 		<th>
	...
	<tr>  <!-- 0 -->
		...
		<td><input type="text" name="remoteIPaddr[]" size="15" maxlength="15"></td>
...
<div class="btn_ctl">
	<input class="link_bg" type="submit" value="<% multilang("245" "LANG_ADD"); %>" name="addPortFw" onClick="return addClick(this)">
...
<div class="column">
	<div class="column_title">
		...
		<p> <% multilang("431" "LANG_CURRENT_PORT_FORWARDING_TABLE"); %></p>
...
<div class="btn_ctl">
	<input class="link_bg" type="submit" value="<% multilang("248" "LANG_DELETE_SELECTED"); %>" name="deleteSelPortFw" onClick="return deleteClick(this)">
	<input class="link_bg" type="submit" value="<% multilang("249" "LANG_DELETE_ALL"); %>" name="deleteAllPortFw" onClick="return deleteAllClick(this)">
</div>
	<script>
