...
{
	if(obj.value==""){		
		alert('<% multilang("2304" "LANG_PLEASE_ENTER_KEY"); %>');
		obj.focus();
		return false;
	}
	for (var i=0; i<obj.value.length; i++) {
		if ((obj.value.charAt(i) >= '0' && obj.value.charAt(i) <= '9')||(obj.value.charAt(i) >= 'a' && obj.value.charAt(i) <= 'f')||(obj.value.charAt(i) >= 'A' && obj.value.charAt(i) <= 'F'))
			continue;		
		alert('<% multilang("2305" "LANG_PLEASE_ENTER_HEXADECIMAL_NUMBER"); %>');
		obj.focus();
		return false;
	}
...
function onESPAuthChange(){
	if(document.ipsec.esp_a_algo.value=="0"){
		if(document.ipsec.esp_e_algo.value!="0"){
			...
			alert('<% multilang("2306" "LANG_NEITHER_ENCRYPT_NOR_AUTH_IS_SELECTED"); %>');
...
function onClickSaveConfig(obj)
{
	...
	if (document.ipsec.IpProtocolType.value == 1){
		if(checkIP(document.ipsec.rtunnelAddr)==false)
			return false;
		if(checkIP(document.ipsec.ltunnelAddr)==false)
			return false;
	}else{
		if (! isGlobalIpv6Address( document.ipsec.rtunnelAddr.value) )
		{			
			alert('<% multilang("2362" "LANG_INVALID_DESTINATION_IPV6_ADDRESS"); %>');
			document.ipsec.rtunnelAddr.focus();
			return false;
		}
		if (! isGlobalIpv6Address( document.ipsec.ltunnelAddr.value) )
		{			
	...
	if(document.ipsec.transMode.value =="0")
	{
		var remoteMaskLen= getDigit(document.ipsec.remotemask.value, 1);
		var localMaskLen= getDigit(document.ipsec.localmask.value, 1);
		...
		if (document.ipsec.IpProtocolType.value == 1)
		{
			...
			if (remoteMaskLen > 32 || remoteMaskLen < 0)
			{				
				alert('<% multilang("2323" "LANG_LENGTH_SHOULD_LESS_THEN_32"); %>');
				document.ipsec.remotemask.focus();
				return false;
			}
			...
			if (localMaskLen > 32 || localMaskLen < 0)
			{				
				alert('<% multilang("2323" "LANG_LENGTH_SHOULD_LESS_THEN_32"); %>');
				document.ipsec.localmask.focus();
				return false;
			}
		}
		else
		{
			if (! isGlobalIpv6Address( document.ipsec.remoteip.value) )
			{			
				alert('<% multilang("2362" "LANG_INVALID_DESTINATION_IPV6_ADDRESS"); %>');
				document.ipsec.remoteip.focus();
				return false;
			}
					
			if (remoteMaskLen > 64 || remoteMaskLen < 0)
			{				
				alert('<% multilang("2263" "LANG_INVALID_SOURCE_IPV6_PREFIX_LENGTH_IT_MUST_BE_0_64"); %>');
				document.ipsec.remotemask.focus();
				return false;
			}
			if (! isGlobalIpv6Address( document.ipsec.localip.value) )
			{			
				alert('<% multilang("2360" "LANG_INVALID_SOURCE_IPV6_ADDRESS"); %>');
				document.ipsec.localip.focus();
				return false;
			}
			if (localMaskLen > 64|| localMaskLen < 0) 
			{				
				alert('<% multilang("2267" "LANG_INVALID_DESTINATION_IPV6_PREFIX_LENGTH_IT_MUST_BE_0_64"); %>');
				document.ipsec.localmask.focus();
				return false;
			}
		}
	...
	if(document.ipsec.negoType[1].checked){
		if(document.ipsec.encapsType.value=="1" || document.ipsec.encapsType.value=="3"){
			...
			if(checkDigit(document.ipsec.spi_out_esp.value)==false){				
				alert('<% multilang("2307" "LANG_SPI_SHOULD_BE_A_DIGIT_NUMBER"); %>');
			...
			if(Number(document.ipsec.spi_out_esp.value)>=0 && Number(document.ipsec.spi_out_esp.value)<=255){				
				alert('<% multilang("2308" "LANG_SPI_0_255_IS_RESERVED"); %>');
				document.ipsec.spi_out_esp.focus();
				return false;
			}
	...
	if(Number(document.ipsec.filterPort.value)<0 && Number(document.ipsec.filterPort.value)>65535){		
		alert('<% multilang("2309" "LANG_PORT_SHOULD_BE_0_65535"); %>');
		document.ipsec.filterPort.focus();
		return false;
	}
	if(document.ipsec.negoType[0].checked){
		if(document.ipsec.psk.value.length > 128){			
			alert('<% multilang("2310" "LANG_PSK_LENGTH_SHOULD_LESS_THAN_128"); %>');
			document.ipsec.filterPort.focus();
			return false;
		}
	...
	<div class="data_common">
		<table>
			...
			<tr id="remoteTunnel">
				<th>&nbsp;&nbsp;Tunnel Addr:</th>
				<td>
					<input name="rtunnelAddr" type="text"  onblur="checkit(this)" onfocus="clearit(this)" />
				</td>
			</tr>
			<tr id="remoteIntIP">
				<th>&nbsp;&nbsp;Internal IPaddr:</th>
				<td>
					<input name="remoteip" type="text" onblur="checkit(this)" onfocus="clearit(this)" />
				</td>
			</tr>
			<tr id="remoteMask">
				<th>&nbsp;&nbsp;Subnet Mask Length:</th>
				<td>
					<input name="remotemask" type="text"  onblur="checkit(this)" onfocus="clearit(this)" />
				</td>
			</tr>
			<tr>
			...
			<tr id="localTunnel">
				<th>&nbsp;&nbsp;Tunnel Addr:</th>
				<td>
					<input name="ltunnelAddr" type="text" onblur="checkit(this)" onfocus="clearit(this)" />
				</td>
			</tr>
			<tr id="localIntIP">
				<th>&nbsp;&nbsp;Internal IPaddr</th>
				<td>
					<input name="localip" type="text" onblur="checkit(this)" onfocus="clearit(this)" />
				</td>
			</tr>
			<tr id="localMask">
				<th>&nbsp;&nbsp;Subnet Mask Length</th>
				<td>
					<input name="localmask" type="text" onblur="checkit(this)" onfocus="clearit(this)" />
				</td>
			</tr>
			<tr>
...
<input name="submit-url" type="hidden" value="/ipsecv6.asp" />
