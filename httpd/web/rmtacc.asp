<%SendWebHeadStr(); %>
<title><% multilang("45" "LANG_REMOTE_ACCESS"); %><% multilang("262" "LANG_CONFIGURATION"); %></title>
...
function addClick(obj)
{
	...
	if (dTelnet == dFtp || dTelnet == dWeb) {
		alert("<% multilang("2443" "LANG_DUPLICATED_PORT_NUMBER"); %>");
	...
	if (document.acc.w_telnet.checked) {
		if (document.acc.w_telnet_port.value=="") {
			alert("<% multilang("2444" "LANG_PORT_RANGE_CANNOT_BE_EMPTY_YOU_SHOULD_SET_A_VALUE_BETWEEN_1_65535"); %>");
			document.acc.w_telnet_port.focus();
			return false;
		}
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("45" "LANG_REMOTE_ACCESS"); %> <% multilang("262" "LANG_CONFIGURATION"); %></p>
	<p class="intro_content"> <% multilang("477" "LANG_THIS_PAGE_IS_USED_TO_ENABLE_DISABLE_MANAGEMENT_SERVICES_FOR_THE_LAN_AND_WAN"); %></p>
</div>
...
<div class="data_vertical data_common_notitle">
	 <div class="data_common ">
		<table>
		<tr>
			<th align=left><% multilang("407" "LANG_SERVICE"); %><% multilang("724" "LANG_NAME"); %></th>
			<th align=center><% multilang("6" "LANG_LAN"); %></th>
			<th align=center><% multilang("11" "LANG_WAN"); %></th>
			<th align=center><% multilang("478" "LANG_WAN_PORT"); %></th>
		</tr>
		<% rmtaccItem(); %>
		</table>
	</div>
...
<div class="btn_ctl">
	<input class="link_bg" type="submit" value="<% multilang("169" "LANG_APPLY_CHANGES"); %>" name="set" onClick="return addClick(this)">
