...
{
	if (ip.value=="") {
		alert("<% multilang("1767" "LANG_INVALID_IPV4_ADDR_SHOULD_NOT_EMPTY"); %>"); 
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( validateKey( ip.value ) == 0 ) {
		alert("<% multilang("1768" "LANG_INVALID_IPV4_ADDR_SHOULD_BE_DECIMAL_NUM"); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	...
	if( IsLoopBackIP( ip.value)==1 ) {
		alert("<% multilang("1769" "LANG_INVALID_IPV4_ADDR"); %>");
	...
	if ( !checkDigitRange(ip.value,1,0,255) ) {
		alert("<% multilang("1770" "LANG_INVALID_IPV4_ADDR_1ST_DIGIT"); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,2,0,255) ) {
		alert("<% multilang("1771" "LANG_INVALID_IPV4_ADDR_2ND_DIGIT"); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,3,0,255) ) {
		alert("<% multilang("1772" "LANG_INVALID_IPV4_ADDR_3RD_DIGIT"); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,4,1,254) ) {
		alert("<% multilang("1773" "LANG_INVALID_IPV4_ADDR_4TH_DIGIT"); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
...
function checkHostIP(ip, checkEmpty)
{
	...
	if (!checkDigitRange(ip.value, 4, 0, 254)) {
		alert("<% multilang("2139" "LANG_INVALID_IP_ADDRESS_RANGE_IN_4TH_DIGIT_IT_SHOULD_BE_0_254"); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
...
function checkNetIP(ip, checkEmpty)
{
	...
	if (!checkDigitRange(ip.value, 4, 0, 255)) {
		alert("<% multilang("2140" "LANG_INVALID_IP_ADDRESS_RANGE_IN_4TH_DIGIT_IT_SHOULD_BE_0_255"); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
...
function checkNetmask(netmask, checkEmpty)
{
	...
	if (checkEmpty == 1 && netmask.value == "") {
		alert("<% multilang("1774" "LANG_INVALID_IPV4_SUBNET_SHOULD_NOT_EMPTY"); %>");
	...
	if (validateKey(netmask.value) == 0) {
		alert("<% multilang("1775" "LANG_INVALID_IPV4_SUBNET_SHOULD_BE_DECIMAL_NUM"); %>");
		netmask.value = netmask.defaultValue;
		netmask.focus();
		return false;
	}
	...
	for (i=1; i<=4; i++) {
		...
		if( !(d==0 || d==128 || d==192 || d==224 || d==240 || d==248 || d==252 || d==254 || d==255 )) {
			alert("<% multilang("1776" "LANG_INVALID_IPV4_SUBNET_DIGIT"); %>");
			netmask.focus();
			return false;
		}
	}
...
function checkIPandMask(ip, mask)
{
        var m,n;
        m = getDigit(mask.value,4);
        n = getDigit(ip.value,4);
        if(n==0 && m==255){
                alert("<% multilang("2665" "LANG_INVALID_IP_SUBNET_MASK_COMBINATION"); %>");
                mask.focus();
                return false;
        }
...
function checkMac(macAddr, checkEmpty)
{
	...
	if (checkEmpty == 1 && macAddr.value.length == 0) {
		alert("<% multilang("1777" "LANG_INVALID_MAC_ADDR_SHOULD_NOT_EMPTY"); %>");
		return false;
	}

	if (macAddr.value.length > 0 && macAddr.value.length < 12) {
		alert("<% multilang("1778" "LANG_INVALID_MAC_ADDR_NOT_COMPLETE"); %>");
		macAddr.focus();
		return false;
	}
	...
	if (macdigit == macAddr.value.length || macAddr.value == "000000000000") {
		alert("<% multilang("1779" "LANG_INVALID_MAC_ADDR"); %>");
		macAddr.focus();
		return false;
	}
	...
	for (i=0; i<macAddr.value.length; i++) {
		if ((macAddr.value.charAt(i) >= '0' && macAddr.value.charAt(i) <= '9') ||
			...
			(macAddr.value.charAt(i) >= 'A' && macAddr.value.charAt(i) <= 'F') )
			continue;
		alert("<% multilang("1780" "LANG_INVALID_MAC_ADDR_SHOULD_BE"); %>");
		macAddr.focus();
		return false;
	}
	return true;
...
{
	if ( !confirm('<% multilang("1783" "LANG_CONFIRM_DELETE"); %>') ) {
		return false;
	}
...
function alertInvalid(fieldname, fieldvalue, additional)
{
	if (additional == undefined)
		alert (fieldname + " " + fieldvalue +  "<% multilang("2047" "LANG_INVALID"); %>");
	else
		alert (fieldname + " " + fieldvalue + "<% multilang("2047" "LANG_INVALID"); %>" + additional + ".");
}
...
{   
  ...
  if ( find_flag != 1 ) {
	alert("<% multilang("1784" "LANG_INVALID_IPV6_PREFIX"); %>");
  	return 0;
  }
...
function isULAipv6Address(address)
{
	if ((address.charAt(0) == 'F' || address.charAt(0) == 'f') &&
	(address.charAt(1) == 'D' || address.charAt(1) == 'd')){
		return true;
	}
...
function ignoreAllButtonClickEvent() {
	var x = document.querySelectorAll("input");
	var i;
	if(window.event.defaultPrevented == false){
		for (i = 0; i < x.length; i++) {
			if (x[i].type == "submit" || x[i].type == "reset"){
				x[i].removeEventListener('click', ignoreAllButtonClickEvent, false);
				x[i].addEventListener('click', clickStopper, false);
			}
		}
	}
	return true;
...
function RegisterClickEvent() {
	...
	//alert("RegisterClickEvent");
	for (i = 0; i < x.length; i++) {
		if (x[i].type == "submit" || x[i].type == "reset"){
			x[i].addEventListener('click', ignoreAllButtonClickEvent, false);
		}
	}
	return true;
}
