...
<% SendWebHeadStr();%>
<title><% multilang("293" "LANG_WLAN_EASY_MESH_TOPOLOGY"); %></title>
<style>
.on {display:on}
.off {display:none}
</style>
...
function print_device_json(object_json) {
	device_counter++;
	htmlString += '<li>';
	htmlString += object_json.device_name + ' | ' + object_json.mac_address + ' | ' + object_json.ip_addr + ' | <input type="button" value="<% multilang("3315" "LANG_EASYMESH_SHOW_DETAILS"); %>" class="link_bg" onClick="showDetailOnClick(' + device_counter.toString() + ')">';
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("293" "LANG_WLAN_EASY_MESH_TOPOLOGY"); %></p>
	<p class="intro_content"><% multilang("294" "LANG_WLAN_EASY_MESH_TOPOLOGY_DESC"); %></p>
</div>
...
<table><tr><td>
<% multilang("295" "LANG_WLAN_EASY_MESH_ROLE_IS_NOT_CONTROLLER"); %>
</td></tr></table>
</div>
</div>
  ...
  <!-- <tr>
	<td width="100%" colspan=2><font size=2><b>
	 <input type="checkbox" name="" value="ON" ONCLICK="">&nbsp;&nbsp;Disable EasyMesh</b>
  	...
  	<tr>
		<th><% multilang("296" "LANG_NETWORK_TOPOLOGY"); %>:</th>
	</tr>
	<tr>
	<td>
