...
<!-- add by zzh for loading-->
<script src="/admin/newjs/jquery-1.7.2.min.js" type="text/javascript"></script>
<script src="/admin/newjs/jquery-ui.min.js" type="text/javascript"></script> 
<script src="/admin/newjs/jquery-loading.js" type="text/javascript"></script>
...
function checkDefaultGW() {
	with (document.forms[0]) {
		if (droute[0].checked == false && droute[1].checked == false && gwStr[0].checked == false && gwStr[1].checked == false) {			
			alert('<% multilang("2048" "LANG_A_DEFAULT_GATEWAY_HAS_TO_BE_SELECTED"); %>');
			return false;
		}
...
function check_dhcp_opts()
{
	with (document.forms[0])
	{
		...
		{
			if (opt60_val.value=="") {				
				alert('<% multilang("2049" "LANG_VENDOR_ID_CANNOT_BE_EMPTY"); %>');
				opt60_val.focus();
				return false;
			}
			if (checkString(opt60_val.value) == 0) {				
				alert('<% multilang("2050" "LANG_INVALID_VENDOR_ID"); %>');
				opt60_val.focus();
				return false;
			}
		}
		...
		{
			if (iaid.value=="") {				
				alert('<% multilang("2051" "LANG_IAID_CANNOT_BE_EMPTY"); %>');
				iaid.focus();
				return false;
			}
			if (checkDigit(iaid.value) == 0) {				
				alert('<% multilang("2052" "LANG_IAID_SHOULD_BE_A_NUMBER"); %>');
				iaid.focus();
				return false;
			}
			...
			if(duid_type[1].checked)
			{
				...
				if (duid_ent_num.value=="") {					
					alert('<% multilang("2053" "LANG_ENTERPRISE_NUMBER_CANNOT_BE_EMPTY"); %>');
					duid_ent_num.focus();
					return false;
				}
				if (checkDigit(duid_ent_num.value) == 0) {					
					alert('<% multilang("2054" "LANG_ENTERPRISE_NUMBER_SHOULD_BE_A_NUMBER"); %>');
					duid_ent_num.focus();
					return false;
				}

				if (duid_id.value=="") {
					alert('<% multilang("2055" "LANG_DUID_IDENTIFIER_CANNOT_BE_EMPTY"); %>');
					duid_id.focus();
					return false;
				}
				if (checkString(duid_id.value) == 0) {					
					alert('<% multilang("2056" "LANG_INVALID_DUID_IDENTIFIER"); %>');
					duid_id.focus();
					return false;
				}
			}
		...
		if(typeof enable_opt_125 !== 'undefined' &&enable_opt_125.checked)
		{
			if (manufacturer.value=="") {				
				alert('<% multilang("2057" "LANG_MANUFACTURER_OUI_CANNOT_BE_EMPTY"); %>');
				manufacturer.focus();
				return false;
			}
			if (checkString(manufacturer.value) == 0) {				
				alert('<% multilang("2058" "LANG_INVALID_MANUFACTURER_OUI"); %>');
				manufacturer.focus();
				return false;
			}

			if (product_class.value=="") {				
				alert('<% multilang("2059" "LANG_PRODUCT_CLASS_CANNOT_BE_EMPTY"); %>');
				product_class.focus();
				return false;
			}
			if (checkString(product_class.value) == 0) {				
				alert('<% multilang("2060" "LANG_INVALID_PRODUCT_CLASS"); %>');
				product_class.focus();
				return false;
			}

			if (model_name.value=="") {
				alert('<% multilang("2061" "LANG_MODEL_NAME_CANNOT_BE_EMPTY"); %>');				
				model_name.focus();
				return false;
			}
			if (checkString(model_name.value) == 0) {				
				alert('<% multilang("2062" "LANG_INVALID_MODEL_NAME"); %>');
				model_name.focus();
				return false;
			}

			if (serial_num.value=="") {				
				alert('<% multilang("2063" "LANG_SERIAL_NUMBER_CANNOT_BE_EMPTY"); %>');
				serial_num.focus();
				return false;
			}
...
function applyCheck(obj)
{
	...
	if( str1 != 0){
  		if ( str2.length != 12) {
			alert('<% multilang("3239" "LANG_INVALID_MAC_CLONE_ADDR_NOT_COMPLETE"); %>');
  		...
  		for (var i=0; i<str2.length; i++) {
			...
			alert('<% multilang("3240" "LANG_INVALID_MAC_CLONE_ADDR_SHOULD_BE"); %>');
			document.ethwan.maccloneaddr.focus();
			return false;
 	 	}
	}
	...
	if (<% checkWrite("wan_logic_port"); %>)
	{
		if(logic_port.value == "")
		{
			alert("<% multilang("3253" "LANG_INCORRECT_WAN_PORT_NUM_SHOULE_NOT_BE_NULL"); %>");
			return false;
		}
	}
	if (document.ethwan.vlan.checked == true) {
		if (document.ethwan.vid.value == "") {			
			alert('<% multilang("2065" "LANG_VID_SHOULD_NOT_BE_EMPTY"); %>');
			document.ethwan.vid.focus();
			return false;
		}
		else if(document.ethwan.vid.value<0 ||document.ethwan.vid.value>4095) {
				alert("<% multilang("2351" "LANG_INCORRECT_VLAN_ID_SHOULE_BE_1_4095"); %>");
				return false;
		}		
	}
	...
	if (document.ethwan.multicast_vid.style.display != "none")
	{
		...
		if (multicast_vid_value.length != 0)
		{
			...
			{
				alert('<% multilang("2790" "LANG_MCAST_INVALID_VLAN_ID"); %>');
				return false;
			}
		}
	}
	...
	if ( document.ethwan.adslConnectionMode.value == 2 ) {
		if (document.ethwan.pppUserName.value=="") {			
			alert('<% multilang("2066" "LANG_PPP_USER_NAME_CANNOT_BE_EMPTY"); %>');
			document.ethwan.pppUserName.focus();
			return false;
		}
		if (includeSpace(document.ethwan.pppUserName.value)) {			
			alert('<% multilang("2067" "LANG_CANNOT_ACCEPT_SPACE_CHARACTER_IN_PPP_USER_NAME"); %>');
			document.ethwan.pppUserName.focus();
			return false;
		}
		if (checkString(document.ethwan.pppUserName.value) == 0) {			
			alert('<% multilang("2068" "LANG_INVALID_PPP_USER_NAME"); %>');
			document.ethwan.pppUserName.focus();
			return false;
		}
		...
		if (document.ethwan.pppPassword.value=="") {			
			alert('<% multilang("2069" "LANG_PPP_PASSWORD_CANNOT_BE_EMPTY"); %>');
			document.ethwan.pppPassword.focus();
			return false;
		}

		if (includeSpace(document.ethwan.pppPassword.value)) {
			alert('<% multilang("2070" "LANG_CANNOT_ACCEPT_SPACE_CHARACTER_IN_PPP_PASSWORD"); %>');
			document.ethwan.pppPassword.focus();
			return false;
		}
		if (checkString(document.ethwan.pppPassword.value) == 0) {
			alert('<% multilang("2071" "LANG_INVALID_PPP_PASSWORD"); %>');
			document.ethwan.pppPassword.focus();
			return false;
		}
		...
		if (document.ethwan.pppConnectType.selectedIndex == 1) {
			if (document.ethwan.pppIdleTime.value <= 0) {				
				alert('<% multilang("2072" "LANG_INVALID_PPP_IDLE_TIME"); %>');
	...
	{
		if( document.ethwan.mtu.value<1280 || document.ethwan.mtu.value>1500 )
		{
			alert("<% multilang("1880" "LANG_STRMRUERR"); %>");
		...
		if (document.ethwan.adslConnectionMode.value != 0
			...
			if(document.ethwan.AddrMode.value == '16') {
				if(document.ethwan.iana.checked == false && document.ethwan.iapd.checked == false ) {
					alert('<% multilang("2074" "LANG_PLEASE_SELECT_IANA_OR_IAPD"); %>');
			...
			if(document.ethwan.AddrMode.value == '2') {
				if(document.ethwan.Ipv6Addr.value == "" || document.ethwan.Ipv6PrefixLen.value == "") {
					alert('<% multilang("2075" "LANG_PLEASE_INPUT_IPV6_ADDRESS_AND_PREFIX_LENGTH"); %>');
				...
				if(document.ethwan.Ipv6Addr.value != ""){
					...
					if (prefixlen > 128 || prefixlen <= 0) {
						alert('<% multilang("2077" "LANG_INVALID_IPV6_PREFIX_LENGTH"); %>');
				...
				if(document.ethwan.Ipv6Gateway.value != "" ){
					if (! isUnicastIpv6Address( document.ethwan.Ipv6Gateway.value) ){
						alert('<% multilang("2078" "LANG_INVALID_IPV6_GATEWAY_ADDRESS"); %>');
				...
				if(document.ethwan.Ipv6Dns1.value != "" ){
					if (! isIpv6Address( document.ethwan.Ipv6Dns1.value) ){
						alert('<% multilang("2079" "LANG_INVALID_PRIMARY_IPV6_DNS_ADDRESS"); %>');
				...
				if(document.ethwan.Ipv6Dns2.value != "" ){
					if (! isIpv6Address( document.ethwan.Ipv6Dns2.value) ){
						alert('<% multilang("2080" "LANG_INVALID_SECONDARY_IPV6_DNS_ADDRESS"); %>');
			...
			else if (document.ethwan.IpProtocolType.value == 2 || document.ethwan.IpProtocolType.value == 3) {
				...
				if (document.ethwan.v6TunnelType.value == 1) // 6rd(Manual)
				{
					if (document.ethwan.SixrdMode[1].checked) {
						if(document.ethwan.SixrdBRv4IP.value == ""){
							alert('<% multilang("2081" "LANG_INVALID_6RD_BOARD_ROUTER_V4IP_ADDRESS"); %>');
							document.ethwan.SixrdBRv4IP.focus();
							return false;
						}

						if(document.ethwan.SixrdIPv4MaskLen.value == ""){
							alert('<% multilang("2082" "LANG_INVALID_6RD_IPV4_MASK_LENGTH"); %>');
							document.ethwan.SixrdIPv4MaskLen.focus();
							return false;
						}

						if(document.ethwan.SixrdPrefix.value == ""){
							alert('<% multilang("2083" "LANG_INVALID_6RD_PREFIX_ADDRESS"); %>');
							document.ethwan.SixrdPrefix.focus();
							return false;
						}

						if(document.ethwan.SixrdPrefixLen.value == ""){
							alert('<% multilang("2084" "LANG_INVALID_6RD_PREFIX_LENGTH"); %>');
...
function deleteCheck(obj)
{
	...
	{		
		alert('<% multilang("2088" "LANG_NO_LINK_SELECTED"); %>');
		return false;
	}
...
function ipv6SettingsEnable(UseDefaultSetting)
{
	if(document.ethwan.IpProtocolType.value != 1){
		...
		if(<% checkWrite("napt_v6"); %>){
			document.getElementById('tbnaptv6').style.display="block";
			NATv6Change();
		}
	...
	with ( document.forms[0] )
	{
		...
		if(itlk == null)
		{
			...
			if (isLocalWan()) //TR069, VOIP and TR069_VOIP
				...
				document.getElementById("div_pmap").style.display = "block";
...
//					}
				...
				if (itlk.cmode == 1 || itlk.cmode == 8 || itlk.cmode ==9 || itlk.cmode ==10 )//IPoE or 6RD or 6in4
				{
					if (itlk.ipDhcp == 1)
					{
...
<style>
    /* Force the page to allow vertical scrolling inside the iframe */
    html, body {
...
<div class="intro_main ">
	...
	<p class="intro_content"> <% multilang("307" "LANG_PAGE_DESC_CONFIGURE_PARAMETERS"); %><% getWanIfDisplay(); %><% multilang("11" "LANG_WAN"); %></p>
</div>
<form action=/boaform/formWanEth method=POST name="ethwan">
<input type=hidden name=v6TunnelRv4IP value="">
<table border="0" cellspacing="4" width="800" <% WANConditions(); %>>
	<tr>
		<td>
			<b><% multilang("308" "LANG_WAN_MODE"); %>:</b>
...
<table>
	...
	<tr id="multicast_vid_tr" style="display:none">
		<th><% multilang("2788" "LANG_MCAST_VLAN"); %> ID: [1-4095]</th>
	...
	<tr>
		<th><% multilang("318" "LANG_ADMIN_STATUS"); %>:</th>
		<td><input type=radio value=1 name="chEnable"><% multilang("272" "LANG_ENABLE"); %>
			<input type=radio value=0 name="chEnable" checked><% multilang("271" "LANG_DISABLE"); %>
		</td>
	</tr>
	<% ShowConnectionType(); %>
	...
	<tr id=tb_macclone style="display:none">
    	<th>Mac <% multilang("3238" "LANG_CLONE_ADDRESS"); %>:  </th>
    	<td>
    	<input type="text" name="maccloneaddr" size="12" maxlength="15" >
    	</td>
  	</tr>
	<tr ID=dgwshow style="display:none">
		<th><% multilang("317" "LANG_DEFAULT_ROUTE"); %>:</th>
		<td>
			<input type=radio value=0 name="droute"><% multilang("271" "LANG_DISABLE"); %>
			<input type=radio value=1 name="droute" checked><% multilang("272" "LANG_ENABLE"); %>
		</td>
	</tr>
...
<input class="link_bg" type="submit" value="<% multilang("169" "LANG_APPLY_CHANGES"); %>" name="apply" onClick="return applyCheck(this)">&nbsp; &nbsp; &nbsp; &nbsp;
<input class="link_bg" type="submit" value="<% multilang("335" "LANG_DELETE"); %>" name="delete" onClick="return deleteCheck(this)">
