...
<%SendWebHeadStr(); %>
<title><% multilang("666" "LANG_ADDRESS_MAPPING_RULE"); %><% multilang("262" "LANG_CONFIGURATION"); %></title>
	 ...
	 if ( document.addressMap.addressMapType.selectedIndex == 0 ) {
		...
		//alert('You select Many-to-One.');  		
  		...
  		if ( ls1 != le1 || ls2 != le2 || ls3 != le3 ) {
  			alert("<% multilang("2399" "LANG_LOCAL_START_IP_DOMAIN_IS_DIFFERENT_FROM_END_IP"); %>");
  			document.addressMap.lsip.focus();
  			return false;
  		}  		 		
  		
  		if ( le <= ls ) {
  			alert("<% multilang("2108" "LANG_INVALID_LOCAL_IP_RANGE"); %>");
  			document.addressMap.lsip.focus();
  			return false;
  		} 		
		...
		if ( !checkIP(document.addressMap.lsip)) {  		
  		...
  		if ( gs1 != ge1 || gs2 != ge2 || gs3 != ge3 ) {
  			alert("<% multilang("2110" "LANG_GLOBAL_START_IP_DOMAIN_IS_DIFFERENT_FROM_GLOBAL_END_IP"); %>");
  		...
  		if ( ge <= gs ) {
  			alert("<% multilang("2111" "LANG_INVALID_GLOBAL_IP_RANGE"); %>");
  			document.addressMap.gsip.focus();
  			return false;
  		}
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("666" "LANG_ADDRESS_MAPPING_RULE"); %> <% multilang("262" "LANG_CONFIGURATION"); %></p>
	<p class="intro_content"> <% multilang("667" "LANG_THIS_PAGE_IS_USED_TO_SET_AND_CONFIGURE_THE_ADDRESS_MAPPING_RULE_FOR_YOUR_DEVICE"); %> <% multilang("668" "LANG_THE_MAXIMUM_NUMBER_OF_ENTRIES_ARE_16"); %></p>
</div>
...
<div class="data_common data_common_notitle">
	<table>
	  <tr>
	      <th><% multilang("320" "LANG_TYPE"); %>:</th>
	      <td>
		      <select size="1" name="addressMapType" onChange="checkState()">
			      <option value=1><% multilang("669" "LANG_ONE_TO_ONE"); %></option>
			      <option value=2><% multilang("670" "LANG_MANY_TO_ONE"); %></option>
			      <option value=3><% multilang("671" "LANG_MANY_TO_MANY_OVERLOAD"); %></option>
			      <option value=4><% multilang("672" "LANG_ONE_TO_MANY"); %></option>
		      </select>
	      </td>
	  </tr>  
	  <tr>
	      <th><% multilang("673" "LANG_LOCAL_START_IP"); %>:</th>
	      <td><input type="text" name="lsip" size="15" maxlength="15" value=<% getInfo("local-s-ip"); %>></td>
	  </tr>  
	  <tr>
	      <th><% multilang("674" "LANG_LOCAL_END_IP"); %>:</th>
	      <td><input type="text" name="leip" size="15" maxlength="15" value=<% getInfo("local-e-ip"); %>></td>
	  </tr> 
	  <tr>
	      <th><% multilang("675" "LANG_GLOBAL_START_IP"); %>:</th>
	      <td><input type="text" name="gsip" size="15" maxlength="15" value=<% getInfo("global-s-ip"); %>></td>
	  </tr>
	  <tr>
	      <th><% multilang("676" "LANG_GLOBAL_END_IP"); %>:</th>
...
<div class="column">
	<div class="column_title">
		<div class="column_title_left"></div>
			<p><% multilang("677" "LANG_CURRENT_ADDRESS_MAPPING_TABLE"); %></p>
...
<div class="btn_ctl">
	<input class="link_bg" type="submit" value="<% multilang("248" "LANG_DELETE_SELECTED"); %>" name="deleteSelAddressMap" onClick="return deleteClick(this)">&nbsp;&nbsp;
	<input class="link_bg" type="submit" value="<% multilang("249" "LANG_DELETE_ALL"); %>" name="deleteAllAddressMap" onClick="return deleteAllClick(this)">&nbsp;&nbsp;&nbsp;
	<input type="hidden" value="/multi_addr_mapping.asp" name="submit-url">
	<input type="hidden" name="postSecurityFlag" value="">
</div>
<script>
