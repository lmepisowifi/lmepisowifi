...
<% SendWebHeadStr();%>
<title><% multilang("281" "LANG_WLAN_EASY_MESH_INTERFACE_SETUP"); %></title>
<style>
.on {display:on}
.off {display:none}
</style>
...
function loadInfo()
{
	...
	if (role == 0) {
		...
		document.getElementById("wsc_trigger").innerHTML   = '<th width="30%">'+'<% multilang("284" "LANG_WLAN_WPS_TRIGGER"); %>'+':</th><td width="70%"><input type="submit" value="'+'<%multilang("265" "LANG_START_PBC"); %>'+'" class="link_bg" name="start_wsc" onClick="return wpsTrigger(this)"></td>';
...
function saveChanges(obj)
{
	if (!document.getElementById("role_disabled").checked) {
		if ("" == document.getElementById("device_name_text").value) {
			alert("<% multilang("287" "LANG_WLAN_EASY_MESH_DEVICE_NAME_CANNOT_BE_EMPTY"); %>");
			return false;
		}

		if (document.getElementById("device_name_text").value.length > 32) {
			alert("<% multilang("288" "LANG_WLAN_EASY_MESH_DEVICE_NAME_SHOULD_BE_LESS_OR_EQUAL_TO_32_CHARACTERS"); %>");
			return false;
		}
	}
	...
	if (dot11kvDisabled && (!document.getElementById("role_disabled").checked)) {
		if(!confirm("<% multilang("289" "LANG_WLAN_EASY_MESH_11KV_ENABLE_WARNING_MESSAGE"); %>")){
			return false;
		}
	...
	//	alert("<% multilang("290" "LANG_WLAN_EASY_MESH_SECURTITY_WRONG_MESSAGE"); %>");
	//	return false;
	//}

	if (<% checkWrite("needPopupBackhaul"); %> && (role == 1)) {
		if(!confirm("<% multilang("291" "LANG_WLAN_EASY_MESH_ALERT_VAP1_AUTO_MANAGED_MESSAGE"); %>")){
			return false;
		}
	}
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("281" "LANG_WLAN_EASY_MESH_INTERFACE_SETUP"); %></p>
	<p class="intro_content"><% multilang("292" "LANG_WLAN_EASY_MESH_DESC"); %></p>
</div>
...
<table>
	...
	<tr id="device_name">
		<th width="30%"><% multilang("105" "LANG_DEVICE_NAME"); %>:</th>
		<td width="70%">
	...
	<tr id="is_controller">
		<th width="30%"><% multilang("283" "LANG_ROLE"); %>:</th>
		<td width="70%">
		<input type="radio" id="role_controller" name="role" value="controller" onclick="isControllerOnChange()"><% multilang("285" "LANG_CONTROLLER"); %>&nbsp;&nbsp;
		<div id="agent_div" style="display:none">
		<input type="radio" id="role_agent" name="role" value="agent" onclick="isControllerOnChange()"><% multilang("286" "LANG_AGENT"); %>&nbsp;&nbsp;
		</div>
		<input type="radio" id="role_disabled" name="role" value="disabled" onclick="isControllerOnChange()"><% multilang("201" "LANG_DISABLED"); %></td>
	</tr>
