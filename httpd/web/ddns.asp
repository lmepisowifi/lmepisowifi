...
<title><% multilang("399" "LANG_DYNAMIC"); %> DNS <% multilang("262" "LANG_CONFIGURATION"); %></title>
...
function btnClick()
{
	if (document.ddns.hostname.value=="") {		
		alert('<% multilang("2141" "LANG_PLEASE_ENTER_HOSTNAME_FOR_THIS_ACCOUNT"); %>');
		document.ddns.hostname.focus();
		deSelected();
		return false;
	}
	if (includeSpace(document.ddns.hostname.value)) {		
		alert('<% multilang("2142" "LANG_INVALID_HOST_NAME"); %>');
		document.ddns.hostname.focus();
		return false;
	}
	...
	if (document.ddns.ddnsProv.value=="0") {
		if (document.ddns.username.value=="") {			
			alert('<% multilang("2143" "LANG_PLEASE_ENTER_USERNAME_FOR_THIS_ACCOUNT"); %>');
		...
		if (document.ddns.password.value=="") {			
			alert('<% multilang("2144" "LANG_PLEASE_ENTER_PASSWORD_FOR_THIS_ACCOUNT"); %>');
	...
	if (document.ddns.ddnsProv.value=="1") {
		if (document.ddns.email.value=="") {
			alert('<% multilang("2145" "LANG_PLEASE_ENTER_EMAIL_FOR_THIS_ACCOUNT"); %>');			
			document.ddns.email.focus();
			deSelected();
			return false;
		}
		if (includeSpace(document.ddns.email.value)) {			
			alert('<% multilang("2146" "LANG_INVALID_EMAIL"); %>');
			document.ddns.email.focus();
			return false;
		}
		...
		if (document.ddns.key.value=="") {			
			alert('<% multilang("2147" "LANG_PLEASE_ENTER_KEY_FOR_THIS_ACCOUNT"); %>');
			document.ddns.key.focus();
			deSelected();
		        return false;
	        }
		if (includeSpace(document.ddns.key.value)) {			
			alert('<% multilang("2148" "LANG_INVALID_KEY"); %>');
			document.ddns.key.focus();
			return false;
		}
...
{
	if (!selected) {		
		alert('<% multilang("2149" "LANG_PLEASE_SELECT_AN_ENTRY_TO_MODIFY"); %>');
		return false;
	}
	//return addClick();
	if(btnClick())
...
{
	if (!selected) {		
		alert('<% multilang("2150" "LANG_PLEASE_SELECT_AN_ENTRY_TO_DELETE"); %>');
		return false;
	}
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("399" "LANG_DYNAMIC"); %> DNS <% multilang("262" "LANG_CONFIGURATION"); %></p>
	<p class="intro_content"><% multilang("400" "LANG_PAGE_DESC_CONFIGURE_DYNAMIC_DNS_ADDRESS"); %></p>
</div>
...
<div class="data_common data_common_notitle">
	<table>
		...
		<tr>
			<th width="30%">DDNS <% multilang("401" "LANG_PROVIDER"); %>:</th>
			<td width="70%">
...
<div class="column">
	<div class="column_title">
		<div class="column_title_left"></div>
			<p>DynDns/No-IP <% multilang("404" "LANG_SETTINGS"); %></p>
	...
	<div class="data_common">
		<table>
			...
			<tr>
				...
				<input type="checkbox" name="pass_check" onClick="show_password(1)">Show Password
				</td>
			</tr>
			<% ddnsServicePort(); %>
		</table>
...
<div class="column">
	<div class="column_title">
		<div class="column_title_left"></div>
			<p>TZO <% multilang("404" "LANG_SETTINGS"); %></p>
		<div class="column_title_right"></div>
	</div>
	<div class="data_common">
		<table>
			<tr>
				<th width="30%"><% multilang("405" "LANG_EMAIL"); %>:</th>
				<td width="70%"><input type="text" name="email" size="35" maxlength="35"></td>
			</tr>
			<tr>
				<th width="30%"><% multilang("267" "LANG_KEY"); %>:</th>
				<td width="70%">
				<input type="password" name="key" size="35" maxlength="35">
				<input type="checkbox" name="key_check" onClick="show_password(2)">Show Password
				</td>
			</tr>
		</table>
	</div>
...
<div class="btn_ctl">
	...
	<input type="submit" value="<% multilang("245" "LANG_ADD"); %>" name="addacc" onClick="return addClick()" class="link_bg">
	<input type="submit" value="<% multilang("358" "LANG_MODIFY"); %>" name="modify" onClick="return modifyClick()" class="link_bg">
	<input type="submit" value="<% multilang("406" "LANG_REMOVE"); %>" name="delacc" onClick="return removeClick()" class="link_bg">
	<input type="submit" value="<% multilang("474" "LANG_UPDATE"); %>" name="update" onClick="return updateClick()" class="link_bg">
</div>
<div class="column">
	<div class="column_title">
		<div class="column_title_left"></div>
			<p><% multilang("399" "LANG_DYNAMIC"); %> DNS <% multilang("444" "LANG__TABLE"); %></p>
		<div class="column_title_right"></div>
	</div>
