 ...
 * 404to302:index.html # Instead of a 404, send a 302 redirect to index.html
...
struct globals {
	...
	char *loc_ip_str;       /* for $SERVER_ADDR and $SERVER_PORT (local socket addr:port) */
	...
	/* "404to302:PAGE" config directive: redirect (302) to PAGE
	 * instead of returning a 404 Not Found */
	const char *redirect_404;
	smallint redirect_404_active; /* set when a 404 is being turned into a 302 */
#if ENABLE_FEATURE_HTTPD_ACL_IP
...
#define redirect_404        (G.redirect_404       )
#define redirect_404_active (G.redirect_404_active)
...
#define loc_ip_str        (G.loc_ip_str       )
...
static int parse_conf(const char *path, int flag)
{
	...
	while (fgets(buf, sizeof(buf), f) != NULL) {
		...
		/* "404to302:PAGE" - respond to a "not found" URL with a
		 * 302 redirect to PAGE instead of a 404. Only honored in the
		 * root config file. Matched here (before the alphabetic 'ch'
		 * handling is discarded) because the directive begins with a
		 * digit, not a letter. */
		if (flag == FIRST_PARSE
		 && strncmp(buf, "404to302", 8) == 0
		 && buf[8] == ':'
		) {
			free((char*)redirect_404);
			redirect_404 = xstrdup(after_colon);
			continue;
		}
...
static void send_headers(unsigned responseNum)
{
	...
	/* "404to302:PAGE" config directive: turn any 404 Not Found into a
	 * 302 redirect to the configured page. Done before the response
	 * table lookup so the 302 strings/headers are used below. */
	redirect_404_active = 0;
	if (responseNum == HTTP_NOT_FOUND && redirect_404) {
		found_moved_temporarily = redirect_404;
		redirect_404_active = 1;
	...
	if (responseNum == HTTP_MOVED_TEMPORARILY) {
		 ...
		 */
		/* For the directory-redirect case we append "/" (asking the
		 * client to repeat with a trailing slash). For the 404to302
		 * case we redirect verbatim to the configured page, so no "/"
		 * is appended and the original query string is dropped. */
		len += snprintf(iobuf + len, IOBUF_SIZE-3 - len,
				"Location: %s%s%s%s\r\n",
				found_moved_temporarily,
				(redirect_404_active ? "" : "/"),
				(!redirect_404_active && g_query ? "?" : ""),
				(!redirect_404_active && g_query ? g_query : "")
		);
		if (len > IOBUF_SIZE-3)
			len = IOBUF_SIZE-3;
	}
		...
		int post_len)
{
	...
	/* Expose the local (server) IP and port the client connected to, so
	 * CGI scripts (e.g. a captive-portal redirect.sh) can build absolute
	 * URLs pointing at *this* portal without hardcoding an IP. Split the
	 * "IP:PORT" string the same way REMOTE_ADDR is handled above. */
	{
		char *p = loc_ip_str ? loc_ip_str : (char*)"";
		char *cp = strrchr(p, ':');
		if (ENABLE_FEATURE_IPV6 && cp && strchr(cp, ']'))
			cp = NULL;
		if (cp) *cp = '\0'; /* delete :PORT */
...
#if ENABLE_FEATURE_HTTPD_CGI
	/* Capture the LOCAL end of the connection (the IP:port the client
	 * actually connected to) so CGI scripts can learn the portal address
	 * dynamically via $SERVER_ADDR / $SERVER_PORT. The connected socket
	 * is on fd 0 (stdin). This is exactly what a captive portal needs:
	 * the address is whatever interface/IP served this request, with no
	 * hardcoding. */
	{
		len_and_sockaddr locAddr;
		locAddr.len = sizeof(locAddr.u);
		if (getsockname(0, &locAddr.u.sa, &locAddr.len) == 0) {
			loc_ip_str = xmalloc_sockaddr2dotted(&locAddr.u.sa);
		}
	}
#endif
	if (verbose) {
