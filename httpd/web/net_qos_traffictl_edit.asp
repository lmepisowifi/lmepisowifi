...
<%SendWebHeadStr(); %>
<title><% multilang("826" "LANG_ADD_IP_QOS_TRAFFIC_SHAPING_RULE"); %></title>
...
function on_apply() {
	with(document.forms[0]) {
		...
		{
			inflist.focus();
			alert("<% multilang("2387" "LANG_WAN_INTERFACE_NOT_ASSIGNED"); %>");
			return;
		}
		if(srcip.value != "" && sji_checkip(srcip.value) == false)
		{
			srcip.focus();
			alert("<% multilang("2388" "LANG_SOURCE_IP_INVALID"); %>");
			return;
		}
		...
		{
			dstip.focus();
			alert("<% multilang("2389" "LANG_DESTINATION_IP_INVALID"); %>");
			return;
		}
		...
		{
			srcnetmask.focus();
			alert("<% multilang("2390" "LANG_SOURCE_IP_MASK_INVALID"); %>");
			return;
		}
		...
		{
			dstnetmask.focus();
			alert("<% multilang("2391" "LANG_DESTINATION_IP_MASK_INVALID"); %>");
			return;
		}
		if(sport.value <0 || sport.value > 65536)
		{
			sport.focus();
			alert("<% multilang("2392" "LANG_SOURCE_PORT_INVALID"); %>");
			return;
		}
		if (sport.value > 0 && sport.value < 65535)
		{
			if (protolist.value!= 1 && protolist.value!= 2 && protolist.value!= 4) {
				sport.focus();
				alert("<% multilang("2393" "LANG_PLEASE_ASSIGN_TCP_UDP"); %>");
				return;
			}
		}
		...
		{
			dport.focus();
			alert("<% multilang("2394" "LANG_DESTINATION_PORT_INVALID"); %>");
			return;
		}
		...
		{
			rate.focus();
			alert("<% multilang("2395" "LANG_UPLINK_RATE_INVALID"); %>");
			return;
		}		
...
<div id='ipprotbl' style="display:none" class="data_common data_common_notitle">
	<table>								
		<tr><th width="30%">IP <% multilang("727" "LANG_VERSION"); %>:</th>
		...
		<tr>
			...
			<td width="70%">
	  			<select name="direction">
					<option value="0"><% multilang("543" "LANG_UPSTREAM");%></option>
				<% checkWrite("rtk_dev_ap_comment_start"); %>
					<option value="1"><% multilang("542" "LANG_DOWNSTREAM");%> </option>
...
<div id='vidDiv' style="display:none" class="data_common data_common_notitle">			
	<table>			
		<tr>
			 <th width="30%"><% multilang("311" "LANG_VLAN"); %> ID:</th>
...
<div id='ssidDiv' style="display:none" class="data_common data_common_notitle">			
	<table>			
		<tr>
			 <th><% multilang("153" "LANG_SSID"); %> ID:</th>
...
<div class="data_common data_common_notitle">
	<table>		
		<tr> 
			<th><div id='wan_interface'><% multilang("75" "LANG_INTERFACE"); %>:</div></th>
...
<div id='ip6tbl' style="display:none;" class="data_common data_common_notitle">
	<table>
		...
		<tr>
	  		<th><% multilang("827" "LANG_SOURCE_PREFIX_LENGTH"); %>:</th>
		...
		<tr>
	  		<th><% multilang("828" "LANG_DESTINATION_PREFIX_LENGTH"); %>:</th>
...
<div class="data_common data_common_notitle">	
	<table>
		...
		<tr>
	  		<th><% multilang("422" "LANG_DESTINATION_PORT"); %>:</th>
			<td><input type="text" name="dport" size="6" style="width:80px"></td>
		</tr>
		
		<tr>
			 <th><% multilang("817" "LANG_RATE_LIMIT"); %>:</th>
			<td><input type="text" name="rate" size="6" style="width:80px"> kb/s</td>
		</tr>
	</table>
</div>
<div class="btn_ctl">
	<input class="link_bg" type="button" name="return" value="<% multilang("766" "LANG_CLOSE"); %>" onClick="location.href='/net_qos_traffictl.asp';">
	<input class="link_bg" type="button" name="apply" value="<% multilang("169" "LANG_APPLY_CHANGES"); %>" onClick="on_apply();">
