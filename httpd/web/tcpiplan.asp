<% SendWebHeadStr();%>
<title><% multilang("123" "LANG_LAN_INTERFACE_SETTINGS"); %></title>
...
function saveChanges()
{
	...
	with (document.forms[0])
	{
		...
		if (ipv6LLAIPmode.value == 1 ) {// Static mode
			if ( isLinkLocalIpv6Address(Ipv6LLAIP.value) == 0) {
				alert("Invalid LAN IPv6 Link local IP address!");	//Invalid LAN IPv6 LLA address!
				Ipv6LLAIP.focus();
				return false;
			}
		}
	    ...
	    if(RTKIPv6Enable == "yes")
	    {
		    	if ( validateKeyV6IP(ipv6_addr.value) == 0) {
		    		alert("<% multilang("2484" "LANG_INVALID_LAN_IPV6_IP"); %>");
		    		ipv6_addr.focus();
		    		return false;
		    	}
...
function LLAIPModeChange()
{
	with (document.forms[0])
	{
		if (ipv6LLAIPmode.value == 0)
			v6LLAIP_Staic.style.display = 'none';
		else
			v6LLAIP_Staic.style.display = '';
	}
}
<% DHCPClientScript(); %>
<% TcpipLanOninit(); %>
...
<BODY onLoad="on_init();">
	<div class="intro_main ">
		<p class="intro_title"><% multilang("123" "LANG_LAN_INTERFACE_SETTINGS"); %></p>
		<p class="intro_content"><% multilang("124" "LANG_PAGE_DESC_CONFIG_DEVICE_LAN_INTERFACE"); %></p>
	</div>
	...
	<form action=/boaform/formTcpipLanSetup method=POST name="tcpip">
		<div class="data_common data_common_notitle">
			<table>
				...
				<tr>
					<th width="30%"><% multilang("110" "LANG_IPV6_LINK_LOCAL_ADDRESS"); %> <% multilang("151" "LANG_MODE"); %>:&nbsp;&nbsp;</th>
					<td width="70%">
					<select name="ipv6LLAIPmode" onChange="LLAIPModeChange()">
						<option value="0">Auto </option>
						<option value="1">Static</option>
					</select>
					</td>
				</tr>
				<tr id='v6LLAIP_Staic' style="display:none">
					<th width="30%"><% multilang("110" "LANG_IPV6_LINK_LOCAL_ADDRESS"); %>:&nbsp;&nbsp;</th>
					<td width="70%"><input type="text" name="Ipv6LLAIP" size="30" maxlength="60" value=<% getInfo("lanipv6LLAaddr"); %>></td>
				</tr>

				<tbody id='ipv6_settings_div' style="display:none">
		    <!--Manuel br0 global address maybe used for feature, now hide it  temporarily-->
                    <tr style = "display:none">
                      <th width="30%"><% multilang("109" "LANG_IPV6_ADDRESS"); %>:&nbsp;&nbsp;</th>
                      <td width="70%"><input type="text" name="ipv6_addr" size="30" maxlength="60" value=<% checkWrite("lanipv6addr"); %>></td>
                    </tr>
                    <tr>
                      <th width="30%"><% multilang("143" "LANG_IPV6_DNS_MODE"); %>:&nbsp;&nbsp;</th>
                   ...
                   <option value="1">WANConnection</option>
                    ...
                    <tr id='v6dns_WANConnection' style="display:none">
                 	...
                 	<tr id='v6dns_Staic1' style="display:none">
                  		<th width="30%"><% multilang("5" "LANG_IPV6"); %> <% multilang("879" "LANG_DNS1"); %>:&nbsp;&nbsp;</th>
                 	...
                 	<tr id='v6dns_Staic2' style="display:none">
                        <th width="30%"><% multilang("5" "LANG_IPV6"); %> <% multilang("880" "LANG_DNS2"); %>:&nbsp;&nbsp;</th>
                    ...
                    <tr id="staticipv6prefix" style="display:none">
                     	<th><% multilang("112" "LANG_PREFIX"); %>:&nbsp;&nbsp;</th>
		...
		<script>
		<% initPage("lan"); %>
		document.tcpip.ipv6LLAIPmode.value=<% checkWrite("LAN_LLA_mode"); %>;
		if (document.tcpip.ipv6LLAIPmode.value == 1)
			document.getElementById("v6LLAIP_Staic").style.display = '';
		updateState();
		var  RTKIPv6Enable=<% checkWrite("rtk_ipv6_enable"); %>;
		if (RTKIPv6Enable == "yes")
		{
