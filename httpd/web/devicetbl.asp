...
<%SendWebHeadStr(); %>
<title><% multilang("4" "LANG_DEVICE"); %> <% multilang("776" "LANG_IP_RANGE"); %><% multilang("1188" "LANG_TABLE_2"); %></title>
...
function addClick(obj) 
{
	...
	if (document.ipRange.Opt60.value=="") {		
		alert('<% multilang("2155" "LANG_OPTION60_CANNOT_BE_NULL"); %>');
		document.ipRange.Opt60.focus();
		return false;
	}

	if ( !checkInputIP(document.ipRange.sIp.value ) ) {		
		alert('<% multilang("2156" "LANG_INVALID_SOURCE_RANGE_OF_IP_ADDRESS_IT_SHOULD_BE_IN_IP_POOL_RANGE"); %>');
		document.ipRange.sIp.focus();
		return false;
	}

	if ( !checkInputIP(document.ipRange.dIp.value ) ) {		
		alert('<% multilang("2157" "LANG_INVALID_DESTINATION_RANGE_OF_IP_ADDRESS_IT_SHOULD_BE_IN_IP_POOL_RANGE"); %>');
		document.ipRange.dIp.focus();
		return false;
	}
	...
	if(document.ipRange.dhcpDeviceType.selectedIndex==3) {
		if (document.ipRange.stbdns1.value == "") {			
			alert('<% multilang("2158" "LANG_ENTER_STBS_DNS_VALUE"); %>');
		...
		if (document.ipRange.opchaddr.value == "") {			
			alert('<% multilang("2159" "LANG_ENTER_STBS_OPCH_ADDRESS_VALUE"); %>');
		...
		if (document.ipRange.opchport.value == "") {			
			alert('<% multilang("2160" "LANG_ENTER_STBS_OPCH_PORT_VALUE"); %>');
			document.ipRange.opchport.focus();
			return false;
		}
		if ( !checkDigitRange(document.ipRange.opchport.value,1,1,65535) ) {		  	
			alert('<% multilang("2161" "LANG_INVALID_OPCH_PORT_IT_SHOULD_BE_1_65535"); %>');
			document.ipRange.opchport.focus();
			return false;
		}
	}
	else if(document.ipRange.dhcpDeviceType.selectedIndex!=5)
	{
		if(document.ipRange.OptStr.value == "") {			
			alert('<% multilang("2162" "LANG_ENTER_OPTION_STRING_VALUE"); %>');
			document.ipRange.OptStr.focus();
			return false;
		}
	}
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("4" "LANG_DEVICE"); %> <% multilang("776" "LANG_IP_RANGE"); %><% multilang("1188" "LANG_TABLE_2"); %></p>
	<p class="intro_content"> <% multilang("866" "LANG_THIS_PAGE_IS_USED_TO_CONFIGURE_THE_IP_ADDRESS_RANGE_BASED_ON_DEVICE_TYPE"); %></p>
</div>
...
<form action=/boaform/formIpRange method=POST name="ipRange">
	...
	<div class="data_common data_common_notitle">
		<table>
			<tr>
				<th width=30%><% multilang("4" "LANG_DEVICE"); %><% multilang("724" "LANG_NAME"); %>:&nbsp;&nbsp;</th>
				<td><input type="text" name="name" size="20" maxlength="20"></td>
			</tr>
			<tr>
				<th><% multilang("867" "LANG_START_ADDRESS"); %>:&nbsp;&nbsp;</th>
				<td><input type="text" name="sIp" size="20" maxlength="15"></td>
			</tr>
			<tr>
				<th><% multilang("868" "LANG_END_ADDRESS"); %>:&nbsp;&nbsp;</th>
				<td><input type="text" name="dIp" size="20" maxlength="15"></td>
			</tr>
			<tr>
				<th><% multilang("387" "LANG_GATEWAY_ADDRESS"); %>:&nbsp;&nbsp;</th>
				<td><input type="text" name="gwIp" size="20" maxlength="15"></td>
			</tr>
			<tr>
				<th><% multilang("869" "LANG_OPTION60"); %>:&nbsp;&nbsp;</th>
			...
			<tr>
				<th width=30%><% multilang("870" "LANG_DEVICE_TYPE"); %>:&nbsp;&nbsp;</th>
				<td><select size="1" name="dhcpDeviceType" onChange="deviceTypeSelect()">
					<option selected value=0><% multilang("871" "LANG_PC"); %></option>
					<option value=1><% multilang("872" "LANG_CAMERA"); %></option>
					<option value=2><% multilang("873" "LANG_HGW"); %></option>
					<option value=3><% multilang("874" "LANG_STB"); %></option>
					<option value=4><% multilang("875" "LANG_PHONE"); %></option>
					<option value=100><% multilang("876" "LANG_UNKNOWN"); %></option></select>
				</td>
			</tr>
		</table>
		...
		<table id="option241" style="display:">
			<tr>
				<th width=30%><% multilang("877" "LANG_RESERVED_OPTION"); %>:&nbsp;&nbsp;</th>
			...
			<tr>
				<th><% multilang("878" "LANG_OPTION_STRING"); %></th>
		...
		<table id="option240" style="display:none">	
			...
			<tr>
				<th><% multilang("879" "LANG_DNS1"); %>:</th>
				<td><input type="text" name="stbdns1" value=<% getInfo("stb-dns1"); %>></td>
			</tr>
			<tr>
				<th><% multilang("880" "LANG_DNS2"); %>:</th>
...
<div class="btn_ctl">
	<input class="link_bg" type="submit" value="<% multilang("245" "LANG_ADD"); %>" name="add" onClick="return addClick(this)">&nbsp;&nbsp;
	<input class="link_bg" type="submit" value="<% multilang("335" "LANG_DELETE"); %>" name="del" onClick="return delClick()">&nbsp;&nbsp;
	<input class="link_bg" type="submit" value="<% multilang("358" "LANG_MODIFY"); %>" name="mod" onClick="return addClick(this)">&nbsp;&nbsp
...
<div class="column">
	<div class="column_title">
		<div class="column_title_left"></div>
			<p><% multilang("4" "LANG_DEVICE"); %> <% multilang("776" "LANG_IP_RANGE"); %><% multilang("1188" "LANG_TABLE_2"); %></p>
