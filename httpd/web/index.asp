...
.header-text-container td {
    ...
    font-weight: bold;
    text-align: right;
    text-shadow: 0 0 8px rgba(255, 255, 255, 0.5), 0 0 3px rgba(255, 255, 255, 0.4);
...
document.write("<s"+"cript type='text/javascript' src='adminMenu.js?v="+Math.random()+"'></scr"+"ipt>");
 ...
 <script>
function logoutFrameLoaded() {
    var iframe = document.getElementById('logout_blind');
    try {
        var doc = iframe.contentDocument || iframe.contentWindow.document;
        var bodyText = doc.body ? doc.body.innerHTML : '';

        if (bodyText.indexOf('logout successfully') !== -1) {
            top.window.location.href = '/admin/login.asp';
        }
    } catch (e) {
        // Cross-origin guard — shouldn't happen since it's the same device
        console.error('logoutFrameLoaded error:', e);
    }
}
...
window.addEventListener('load', function() {
    ...
    for (var i = 0; i < COUNT; i++) {
        ...
        particles.push({
            ...
            alpha: 0.70 + Math.random() * 0.30,  /* was 0.6 + 0.4 */
    ...
    function animate() {
        ...
        ctx.fillStyle = 'rgba(10, 22, 40, 0.10)';  /* was 0.18 — slower fade = brighter trails */
...
function cleanload() {
    ...
    try {
        ...
        rawHtml = doc && doc.body ? doc.body.innerHTML : '';
        
        // If the document is totally empty (and it's not the initial blank '#'), the connection dropped
        var currentSrc = iframe.getAttribute('src');
        if (currentSrc !== '#' && currentSrc !== '' && rawHtml.trim() === '') {
            loadFailed = true;
        }
    } catch(e) {
        // Browser blocked access because it loaded an internal error page (e.g. No Internet)
        loadFailed = true;
        
    }

    // --- IFRAME CRASH / DISCONNECT HANDLER ---
    if (loadFailed) {
        iframe.style.visibility = 'hidden';
        var loadDiv = document.getElementById('loadpagediv');
        loadDiv.className = 'no-spinner';
        loadDiv.querySelector('p').textContent = 'Connection was interrupted or page failed to load.';
        
        // Remove old reload button if it exists
        var oldBtn = document.getElementById('iframe-reload-btn');
        if (oldBtn) oldBtn.remove();

        // Create the Reload button
        var reloadBtn = document.createElement('input');
        reloadBtn.type = 'button';
        reloadBtn.id = 'iframe-reload-btn';
        reloadBtn.value = 'Reload Page';
        reloadBtn.className = 'link_bg';
        reloadBtn.style.marginTop = '15px';
        reloadBtn.style.cursor = 'pointer';

        reloadBtn.onclick = function() {
            reloadBtn.remove();
            loadDiv.className = 'loading'; // Restore the spinning animation
            loadDiv.querySelector('p').textContent = 'Reloading...';
            iframe.src = iframe.src; // Force the iframe to try fetching the page again
        };

        loadDiv.appendChild(reloadBtn);
        
        document.getElementById('shade').style.display = 'block'; setBlur(true);
        loadDiv.style.display = 'block';
        return; // Stop running the rest of cleanload
    }
    // -----------------------------------------

    try {
        // Grab the HTML, but strip out all <script> blocks so we don't false-positive on JS code
        var body = rawHtml.replace(/<script[\s\S]*?<\/script>/gi, '');
    ...
    try {
        ...
        // ADD THIS: Restore visibility now that a real page has loaded successfully!
        iframe.style.visibility = ''; 
...
function customConfirmLogout() {
    // Use customprompt.js if loaded in the iframe, or inline version
    try {
        var loadDiv = document.getElementById('loadpagediv');
        var shade   = document.getElementById('shade');

        loadDiv.className = 'no-spinner';
        loadDiv.querySelector('p').innerHTML = 'Are you sure you want to logout?';

        var btnContainer = document.createElement('div');
        btnContainer.style.marginTop = '15px';

        var btnYes = document.createElement('input');
        btnYes.type = 'button'; btnYes.value = 'Yes';
        btnYes.className = 'link_bg';
        btnYes.style.marginRight = '10px';
        btnYes.style.cursor = 'pointer';

        var btnNo = document.createElement('input');
        btnNo.type = 'button'; btnNo.value = 'No';
...
btnYes.onclick = function() {
    ...
    var loadDiv = document.getElementById('loadpagediv');
    loadDiv.className = 'loading';
    loadDiv.querySelector('p').textContent = 'Logging Out.';
    document.getElementById('shade').style.display = 'block';
    setBlur(true);
    loadDiv.style.display = 'block';

    document.getElementById('cmlogout').submit();
};
...
<body>
<div id="shade"></div>
<iframe name="logout_blind" id="logout_blind" style="display:none;" onload="logoutFrameLoaded()"></iframe>
...
<table class="header-text-container">
  ...
  <tr><td>Logged in as: <script>document.write((<% checkWrite("susername"); %>).replace(/"/g, ''));</script>&nbsp;&nbsp;</td></tr>
  <tr><td>
<form action=/boaform/admin/formLogout method=POST name="cmlogout" id="cmlogout" target="logout_blind">
    <a onclick="customConfirmLogout()" 
       style="color:#ffffff;text-decoration:underline;cursor:pointer;font-weight:bold;text-shadow:0 0 5px rgba(255,255,255,0.5);margin-right:7.5px;">
        Logout?
    </a>
