<% SendWebHeadStr();%>
<title><% multilang("185" "LANG_WLAN_ADVANCED_SETTINGS"); %></title>
...
function validateNum(str)
{
  for (var i=0; i<str.length; i++) {
   	if ( !(str.charAt(i) >='0' && str.charAt(i) <= '9')) {
		alert("<% multilang("2538" "LANG_INVALID_VALUE_IT_SHOULD_BE_IN_DECIMAL_NUMBER_0_9"); %>");
		return false;
  	}
  }
  return true;
...
function saveChanges(obj)
{
  ...
  if (document.advanceSetup.fragThreshold.value == "" || num < 256 || num > 2346) {
  	alert("<% multilang("2539" "LANG_INVALID_VALUE_OF_FRAGMENT_THRESHOLD_INPUT_VALUE_SHOULD_BE_BETWEEN_256_2346_IN_DECIMAL"); %>");
  	document.advanceSetup.fragThreshold.focus();
	return false;
  }
  ...
  if (document.advanceSetup.rtsThreshold.value=="" || num > 2347) {
  	alert("<% multilang("2540" "LANG_INVALID_VALUE_OF_RTS_THRESHOLD_INPUT_VALUE_SHOULD_BE_BETWEEN_0_2347_IN_DECIMAL"); %>");
  	document.advanceSetup.rtsThreshold.focus();
	return false;
  }
  ...
  if (document.advanceSetup.beaconInterval.value=="" || num < min_beacon_interval || num > 1024) {
	if(min_beacon_interval == 100)
		alert("<% multilang("2542" "LANG_INVALID_VALUE_OF_BEACON_INTERVAL_INPUT_VALUE_SHOULD_BE_BETWEEN_100_1024_IN_DECIMAL"); %>");
	else
		alert("<% multilang("2541" "LANG_INVALID_VALUE_OF_BEACON_INTERVAL_INPUT_VALUE_SHOULD_BE_BETWEEN_20_1024_IN_DECIMAL"); %>");
  	document.advanceSetup.beaconInterval.focus();
	return false;
  ...
  if (document.advanceSetup.dtimPeriod.value=="" || num < 1 || num > 255) {
	alert("<% multilang("2543" "LANG_INVALID_VALUE_OF_DTIM_PRERIOD_INPUT_VALUE_SHOULD_BE_BETWEEN_1_255_IN_DECIMAL"); %>");
...
function updateState()
{
   if (document.advanceSetup.wlanDisabled.value == "ON") {
    document.querySelector('.data_common').style.display = 'none';
    document.querySelector('.btn_ctl').style.display = 'none';

    var idx = document.advanceSetup.wlan_idx.value;
  ...
  document.advanceSetup.save.style.display = 'none'; 
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("185" "LANG_WLAN_ADVANCED_SETTINGS"); %></p>
	<p class="intro_content"><% multilang("186" "LANG_PAGE_DESC_WLAN_ADVANCE_SETTING"); %></p>
</div>

<form action=/boaform/admin/formAdvanceSetup method=POST name="advanceSetup" target="save_blind">
	...
	<table>
	    ...
	    <tr <% checkWrite("wlan_qtn_hidden_function"); %> <% checkWrite("wlan_advanced_ax_hidden_func"); %>>
	      <th width="30%"><% multilang("187" "LANG_FRAGMENT_THRESHOLD"); %>:</th>
	    ...
	    <tr <% checkWrite("wlan_advanced_ax_hidden_func"); %>>
	      <th width="30%"><% multilang("188" "LANG_RTS_THRESHOLD"); %>:</th>
	      <td width="70%"><input type="text" name="rtsThreshold" size="10" maxlength="4" value=<% getInfo("rtsThreshold"); %>>(0-2347)</td>
	    </tr>
	    <tr>
	      <th width="30%"><% multilang("189" "LANG_BEACON_INTERVAL"); %>:</th>
	      <td width="70%"><input type="text" name="beaconInterval" size="10" maxlength="4" value=<% getInfo("beaconInterval"); %>> (<% checkWrite("min_beacon_interval"); %>-1024 <% multilang("1002" "LANG_MS"); %>)</td>
	    </tr>
	    <tr>
	      <th width="30%"><% multilang("194" "LANG_DTIM_PERIOD"); %>:</th>
	      <td width="70%"><input type="text" name="dtimPeriod" size="10" maxlength="4" value=<% getInfo("dtimPeriod"); %>> (1-255)</td>
	    </tr>

	    <tr>
	      <th width="30%"><% multilang("190" "LANG_DATA_RATE"); %>:</th>
	      <td width="70%"><select size="1" name="txRate">
				<SCRIPT>
					...
					if(!is_wlan_qtn){
						...
						rate_name =["<% multilang("191" "LANG_AUTO"); %>","1M","2M","5.5M","11M","6M","9M","12M","18M","24M","36M","48M","54M", "MCS0", "MCS1",
						...
						idx=0, i=0;
						document.write('<option value="' + (i) + '">' +'<% multilang("191" "LANG_AUTO"); %>'+ '\n');
						idx++;
						defidx=0;
						if ((band & 8) || (band&4)) {
		...
		<div class="btn_ctl">
			<input type="submit" class="inner_btn" value="<% multilang("169" "LANG_APPLY_CHANGES"); %>"  name="save" onClick="return saveChanges(this)" class="link_bg">&nbsp;&nbsp;
