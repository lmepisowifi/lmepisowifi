...
<%SendWebHeadStr(); %>
<title><% multilang("1268" "LANG_CFM_802_1AG_CONFIGURATION"); %></title>
...
function addClick(action,obj) 
{
	...
	{		
		alert('<% multilang("2228" "LANG_YOU_MUST_SELECT_AN_ENTRY_IN_TABLE"); %>');
		return false;
	}
	...
	for(i = 0 ; i < dot1ag.length ; i++)
	{
		...
		{			
			alert('<% multilang("2232" "LANG_INTERFACE_IS_DUPLICATED"); %>');
	...
	if (document.dot1agConf.md_name.value=="") {		
		alert('<% multilang("2233" "LANG_MD_NAME_CANNOT_BE_EMPTY"); %>');
		document.dot1agConf.md_name.focus();
		return false;
	}
	if (checkString(document.dot1agConf.md_name.value) == 0) {		
		alert('<% multilang("2234" "LANG_INVALID_MD_NAME"); %>');
		document.dot1agConf.md_name.focus();
		return false;
	}
	...
	for(i = 0 ; i < dot1ag.length ; i++)
	{
		...
		{			
			alert('<% multilang("2235" "LANG_MD_LEVEL_IS_DUPLICATED"); %>');
	...
	if (document.dot1agConf.ma_name.value=="") {		
		alert('<% multilang("2236" "LANG_MA_NAME_CANNOT_BE_EMPTY"); %>');
		document.dot1agConf.ma_name.focus();
		return false;
	}
	if (checkString(document.dot1agConf.ma_name.value) == 0) {		
		alert('<% multilang("2237" "LANG_INVALID_MA_NAME"); %>');
		document.dot1agConf.ma_name.focus();
		return false;
	}
	...
	{		
		alert('<% multilang("2238" "LANG_MEP_INDEX_CANNOT_BE_EMPTY"); %>');
		document.dot1agConf.mep_id.focus();
		return false;
	}
	if (checkDigitRange(document.dot1agConf.mep_id.value, 1, 1, 8191) == 0)
	{		
		alert('<% multilang("2239" "LANG_MEP_INDEX_SHOULD_BE_A_NUMBER_BETWEEN_1_AND_8191"); %>');
		document.dot1agConf.mep_id.focus();
		return false;
	}
	obj.isclick = 1;
...
<div class="intro_main ">
	<p class="intro_title"><% multilang("1268" "LANG_CFM_802_1AG_CONFIGURATION"); %></p>
	<p class="intro_content"> <% multilang("1269" "LANG_THIS_PAGE_IS_USED_TO_CONFIGURE_IEEE_802_1AG_WHICH_IS_ALSO_KNOWN_AS_CONNECTIVITY_FAULT_MAMAGMENT"); %></p>
</div>
...
<div class="data_common data_common_notitle">
	<table>
		...
		<tr>
		    <th width=30%><% multilang("1212" "LANG_MD_NAME"); %>:&nbsp;&nbsp;</th>
		    <td><input type="text" name="md_name" size="20" maxlength="43"></td>
		</tr>
		<tr>
			<th width=30%><% multilang("1234" "LANG_MD_LEVEL"); %>:</th>
...
<div class="btn_ctl">
	<input type="submit" value="<% multilang("245" "LANG_ADD"); %>" name="action" onClick="return addClick('add',this)">&nbsp;&nbsp;
	<input type="submit" value="<% multilang("335" "LANG_DELETE"); %>" name="action" onClick="return addClick('delete',this)">&nbsp;&nbsp;
	<input type="submit" value="<% multilang("358" "LANG_MODIFY"); %>" name="action" onClick="return addClick('modify',this)">&nbsp;&nbsp
	<input type="button" value="<% multilang("902" "LANG_CLEAR"); %>" onClick="clear_inputs(1)">&nbsp;&nbsp;
...
<div class="column">
	<div class="column_title">
		<div class="column_title_left"></div>
			<p><% multilang("1271" "LANG_CFM_INSTANCE_TABLE"); %></p>
	...
	<div class="data_common">
		<table id="cfm_tbl">
			<tr>
				...
				<th align=center><% multilang("1212" "LANG_MD_NAME"); %></th>
				<th align=center><% multilang(LANG_MD_LVEL); %></th>
				<th align=center><% multilang("1213" "LANG_MA_NAME"); %></th>
				<th align=center><% multilang("1235" "LANG_MEP_ID"); %></th>
				<th align=center><% multilang("1236" "LANG_CCM_STATUS"); %></th>
			</tr>
		</table>
	</div>
</div>
